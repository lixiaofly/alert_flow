...
class Mysql(object):  
        ...
        def insertReturn(self,sql,value,retkey):
                """
                @summary: 向数据表插入一条记录
                @param sql:要插入的ＳＱＬ格式
                @param value:要插入的记录数据tuple/list
                ...
                """
                self._cursor.execute(sql % value)
                self._cursor.execute("select %s" % retkey)
                result = self._cursor.fetchall()
                if len(result) > 0:
                        ret = result[0].get(retkey,0)
                        if ret == -1:
                                return self.__getInsertId()
                        elif ret:
                                return ret
        ...
        def begin(self, pointName='pname'):
                """ 
                @summary: 开启事务 
                """
                #self._conn.autocommit(0)
                self._cursor.execute("start transaction")
                self._cursor.execute("SAVEPOINT %s"% pointName)

        def end(self,pointName='pname', option='commit'):
                """ 
                @summary: 结束事务 
                """  
                if option=='commit':
                        ...
                        self._cursor.execute("ROLLBACK TO %s" % pointName)
                        #self._conn.rollback()
